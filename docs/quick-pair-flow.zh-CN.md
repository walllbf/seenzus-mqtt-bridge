...
```text
https://seenzus.ai/gatewayka/savantai
```
...
```text
VITE_API_PROXY_TARGET=https://seenzus.ai/gatewayka/savantai
```
...
```bash
curl "https://seenzus.ai/gatewayka/savantai/integrations/ha/web-pairing/session/{sessionId}"
```

查询设备目录：

```bash
curl "https://seenzus.ai/gatewayka/savantai/integrations/ha/bridges/{bridgeId}/device-catalog?refresh=true"
```

查询桥接诊断：

```bash
curl "https://seenzus.ai/gatewayka/savantai/integrations/ha/bridges/{bridgeId}/diagnostics"
```
