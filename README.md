...
- 第二步进入对应模式的专属表单
- 快速配对页无需填写任何内容（直接提交），随后跳转外部 seenzus 页面完成授权；`seenzus API 地址` 字段仅在 HA 个人资料开启「高级模式」后出现（本地联调后端用），普通使用一律走内置生产地址
...
| seenzus API 地址（快速配对） | 仅高级模式可见；创建外部配对会话并自动回写桥接配置 | 内置生产地址 |
...
```text
提交快速配对表单（API 地址走内置生产默认值；高级模式可覆盖为联调地址）
